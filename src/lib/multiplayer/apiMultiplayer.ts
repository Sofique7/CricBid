...
/** Production client: talks to /api/multiplayer (Firebase on server). No browser Firebase config needed. */
export class ApiMultiplayerService implements IMultiplayerService {
  ...
  private startHostTicks(code: string): void {
    ...
    this.timerInterval = setInterval(() => {
      void apiPost<{ snapshot: RoomSnapshot }>(`/api/multiplayer/rooms/${code}/action`, {
        ...
        .then((data) => this.applySnapshot(data.snapshot, null))
        .catch(() => {});
    }, 1000);
  }
  ...
  async resetAuction(roomCode: string, players: Player[], teams: Team[]): Promise<void> {
