...
export const Navbar: React.FC = () => {
  ...
  return (
    <nav className="navbar px-4 md:px-8 py-3.5">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center space-x-2.5">
          ...
          >
            Cric<span style={{ color: '#2B2B2B' }}>Bid</span>
          </span>
          <span
            className="text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full border hidden sm:inline-block"
            style={{ color: '#666666', background: '#F5EFE4', borderColor: '#E7DFD1' }}
          >
            Beta
          </span>
        </Link>
        ...
        <div className="hidden md:flex items-center space-x-3">
          ...
          {userTeam && (
            <div
              className="flex items-center space-x-2.5 px-3.5 py-1.5 rounded-full border text-sm"
              style={{ background: '#FFFDF8', borderColor: '#E7DFD1' }}
            >
              <span
                className="w-2 h-2 rounded-full flex-shrink-0"
                style={{ backgroundColor: userTeam.color }}
              />
              <span className="font-medium text-xs" style={{ color: '#666666' }}>
                {userTeam.shortName}
              </span>
              <span className="font-bold text-xs" style={{ color: '#1E1E1E' }}>
                ₹{userTeam.purse.toFixed(2)} Cr
              </span>
              <span style={{ color: '#E7DFD1' }}>|</span>
          ...
          <button
            onClick={toggleVoice}
            id="nav-voice-toggle"
            title={voiceEnabled ? 'Mute auctioneer voice' : 'Enable auctioneer voice'}
            ...
            style={{
              borderColor: voiceEnabled ? '#E7DFD1' : '#FECACA',
              background: voiceEnabled ? '#FFFDF8' : '#FEF2F2',
              color: voiceEnabled ? '#666666' : '#B91C1C',
            }}
          >
            {voiceEnabled ? (
          ...
          <button
            ...
            className="p-2 rounded-lg border transition-colors duration-150"
            style={{ borderColor: '#E7DFD1', background: '#FFFDF8', color: '#666666' }}
          >
            {soundEnabled ? (
          ...
          {userTeamId && (
            <button
              ...
              className="text-xs font-semibold px-3 py-1.5 rounded-lg border transition-colors duration-150"
      ...
      {isOpen && (
        ...
        >
          ...
          {userTeam && (
            <div
              className="px-3 py-2 rounded-lg flex items-center justify-between text-sm border mt-1"
              style={{ background: '#FFFDF8', borderColor: '#E7DFD1' }}
            >
              <span className="font-medium" style={{ color: '#666666' }}>{userTeam.name}</span>
              <span className="font-bold" style={{ color: '#1E1E1E' }}>₹{userTeam.purse.toFixed(2)} Cr</span>
            </div>
          )}
